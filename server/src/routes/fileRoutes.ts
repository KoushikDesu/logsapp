...
router.get('/download/:messageId', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (range) {
      ...
      const isDownload = req.query.download === 'true' || !file_mime_type?.startsWith('image/');
      res.setHeader('Content-Disposition', `${isDownload ? 'attachment' : 'inline'}; filename="${encodeURIComponent(file_name)}"`);
